...
@pytest.mark.parametrize("configuration,n_rows", [
    (["I"],                                                          1),
    (["I", "II", "III"],                                             3),
...
def test_figure_size_matches_layout(ecg_df, configuration, n_rows):
    ...
    try:
        exp_w, exp_h = _compute_figure_size(
            n_rows, N_SAMPLES, FS,
            plotter.speed, plotter.voltage, plotter.row_spacing,
...
@pytest.mark.parametrize("n_rows,show_calibration", [
    (1, True),
    (1, False),
    (3, True),
    (3, False),
...
@pytest.mark.parametrize("grid_mode,expect_many_lines", [
    ("cm", True),
    (None, False),
])
...
@pytest.mark.parametrize("show_calibration,n_rows", [
    (True,  1),
    (True,  3),
    (False, 1),
    (False, 3),
...
def test_calibration_1mv_text(ecg_df, show_calibration, n_rows):
    ...
    configuration = LEAD_NAMES[:n_rows]
    plotter = ECGPlotter(
        grid_mode=None, print_information=False, show_calibration=show_calibration
    )
    fig = plotter.plot(ecg_df, configuration, sampling_frequency=FS, show=False)
    try:
...
@pytest.mark.parametrize("information,needle", [
...
@pytest.mark.parametrize("stats,label,value_substr", [
...
def test_stats_absent_when_no_print_information(ecg_df):
    ...
    plotter = ECGPlotter(grid_mode=None, print_information=False)
    fig = plotter.plot(
        ecg_df, ["I"], sampling_frequency=FS, show=False,
...
def test_full_ecgstats_all_fields_printed(ecg_df):
    ...
    stats = ECGStats(
        bpm=72.0, snr=25.5, rr_interval_ms=833.0, hrv_ms=45.0,
        pr_interval_ms=160.0, qrs_duration_ms=90.0, qt_interval_ms=400.0,
        qtc_interval_ms=420.0, p_axis_deg=60.0, qrs_axis_deg=-30.0, t_axis_deg=45.0,
    )
    plotter = ECGPlotter(grid_mode=None, print_information=True)
    fig = plotter.plot(ecg_df, ["I"], sampling_frequency=FS, show=False, stats=stats)
    try:
...
@pytest.mark.parametrize("stats,present_labels,absent_labels", [
...
@pytest.mark.parametrize("information,present_strings,absent_strings", [
...
@pytest.mark.parametrize("template_key,expected_n_rows", [
    ("1x1",  1),
    ("1x2",  2),
    ("1x3",  3),
    ("1x4",  4),
...
def test_template_all_lead_labels(ecg_df, template_key):
    ...
    try:
        ...
        for lead in expected_leads:
            assert lead in texts, (
                f"Lead '{lead}' missing from figure texts for template '{template_key}'"
            )
    finally:
        plt.close(fig)
...
def test_lead_label_positions(ecg_df, configuration):
    ...
    * Repeated leads (e.g. "II" in two different rows) are distinguished by y,
    ...
    plotter = ECGPlotter(
        grid_mode=None, print_information=False, show_calibration=False,
        show_leads_labels=True, speed=speed, voltage=voltage, row_spacing=row_spacing,
    )
    fig = plotter.plot(ecg_df, configuration, sampling_frequency=FS, show=False)
    try:
        ax = _ax(fig)
        ...
        for i, leads in enumerate(rows):
            ...
            assert len(row_texts) == len(leads), (
                f"Row {i} ({leads}): expected {len(leads)} labels, found {len(row_texts)}"
            )
            ...
            for j, (x, label) in enumerate(row_texts):
                ...
                assert label == leads[j], (
                    f"Row {i}, slot {j}: expected '{leads[j]}', got '{label}'"
                )
                # Correct horizontal position
                assert abs(x - exp_x) < 1e-9, (
                    f"Row {i}, lead '{label}': expected x={exp_x:.6f} in, got {x:.6f} in"
                )
