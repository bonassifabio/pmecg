...
from pmecg import BackgroundAttentionMap, ECGPlotter, IntervalAttentionMap, LineColorAttentionMap
...
@pytest.mark.parametrize(
    ...
    [
        (
            lambda data: IntervalAttentionMap(
                data=data,
                polarity="positive",
                color="red",
                max_attention_mV=0.5,
            ...
            pd.DataFrame({"I": [0.0, 0.25, 0.5, 0.75, 1.0]}),
            {"0", "1"},
        ),
        (
...
def test_attention_adds_color_scale_and_expands_right_margin(attention_map_factory, attention_df, expected_labels):
    ecg_df = pd.DataFrame({"I": np.zeros(5)})
    plotter = ECGPlotter(
        grid_mode=None,
        show_calibration=False,
