...
def _compute_ecg_stats(
    df: pd.DataFrame,
    sampling_frequency: float = 500.0,
    lead: str = "II",
) -> dict[str, float]:
    """Internal function to compute ECG statistics using neurokit2.
    ...
    -------
    dict[str, float]
        A dictionary mapping ECGStats field names to their computed values.
    """
    import neurokit2 as nk

    # Select lead: case-insensitive search or fallback to the first column
    target_lead = lead.upper()
    if target_lead not in df.columns:
        matching_columns = [col for col in df.columns if col.upper() == target_lead]
        if matching_columns:
            target_lead = matching_columns[0]
        else:
            # Fallback to the first available lead if lead II is not present
            target_lead = df.columns[0]

    signal = df[target_lead].values

    stats: dict[str, float] = {}

    try:
        # Process the signal: cleaning, R-peak detection, and Rate calculation
        # We disable HRV to avoid numpy.trapz issues in newer numpy versions
        signals, info = nk.ecg_process(signal, sampling_rate=sampling_frequency)
        
        # Basic metrics from processed signals
        if "ECG_Rate" in signals.columns:
            stats["bpm"] = float(np.mean(signals["ECG_Rate"].dropna()))

        if "ECG_R_Peaks" in info and len(info["ECG_R_Peaks"]) > 1:
            rpeaks = info["ECG_R_Peaks"]
            rr_intervals = np.diff(rpeaks) / sampling_frequency
            stats["rr_interval_ms"] = float(np.mean(rr_intervals) * 1000.0)
            # Simple SDNN as HRV proxy if full HRV fails
            stats["hrv_ms"] = float(np.std(rr_intervals) * 1000.0)

        # Use the delineated waves already present in 'signals' if available
        # NeuroKit ecg_process runs ecg_delineate by default
        
        # Helper to compute mean interval between two wave points
        def get_mean_interval_ms(start_col: str, end_col: str) -> float | None:
            if start_col in signals.columns and end_col in signals.columns:
                start_indices = np.where(signals[start_col] == 1)[0]
                end_indices = np.where(signals[end_col] == 1)[0]
                
                if len(start_indices) > 0 and len(end_indices) > 0:
                    intervals = []
                    for start in start_indices:
                        # Find the first end index after this start index
                        sub_ends = end_indices[end_indices > start]
                        if len(sub_ends) > 0:
                            # Ensure it's not TOO far (e.g., within 1 second)
                            if sub_ends[0] - start < sampling_frequency:
                                intervals.append(sub_ends[0] - start)
                    
                    if intervals:
                        return float(np.mean(intervals) / sampling_frequency * 1000.0)
            return None

        # PR interval: P-onset to R-onset (or Q-peak if R-onset missing)
        pr = get_mean_interval_ms("ECG_P_Onsets", "ECG_R_Onsets")
        if pr:
            stats["pr_interval_ms"] = pr

        # QRS duration: R-onset to R-offset
        qrs = get_mean_interval_ms("ECG_R_Onsets", "ECG_R_Offsets")
        if qrs:
            stats["qrs_duration_ms"] = qrs

        # QT interval: R-onset to T-offset
        qt = get_mean_interval_ms("ECG_R_Onsets", "ECG_T_Offsets")
        if qt:
            stats["qt_interval_ms"] = qt
            # QTc (Bazett's formula: QTc = QT / sqrt(RR))
            if "rr_interval_ms" in stats:
                rr_s = stats["rr_interval_ms"] / 1000.0
                stats["qtc_interval_ms"] = stats["qt_interval_ms"] / np.sqrt(rr_s)

    except Exception:
        # If neurokit processing fails, return what we have (or empty)
        pass
